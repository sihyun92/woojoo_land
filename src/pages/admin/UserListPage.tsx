...
function UserListPage() {
  ...
  const [postPerPage] = useState(7); //한 페이지에 보여질 아이템 수
  ...
  useEffect(() => {
    ...
    fetchList();
    setCount(products.length); //API로 받은 데이터의 갯수를 아이템의 총 개수 상태로 setCount에 전달
  ...
  }, [currentPage, indexOfLastPost, indexOfFirstPost, products, postPerPage]); //위에 기능이 끝나면 배열 안의 결과들을 한 번 실행
      ...
      <UserListContainer>
        ...
        <UserContainer>
          {currentPosts && products.length > 0 ? (
            ...
            <div>유저 데이터를 불러오고 있습니다.</div> // 데이터가 없는 상태는 해당 문구를 출력한다.
          )}
        </UserContainer>
        <PageNation>
